...
# 2 Analytical period sensitivity analysis --------------------------------
# Best DSEM
model_selection <- read_rds("Results/DESM_model_selection.rds")
best_dsem <- model_selection$model

# loop with start year from 1961 to 2001, end year is fixed as 2022 (reference period 1981-2022)
results <- NULL
for (i in 1961:2001) {
  # i=1961
  data_loop <- data %>%
    filter(year %in% c(i:2022))
...
# save results
write_rds(results, "Results/Sensitivity_analysis_results_analytical_period.rds")


# 3 Results figure --------------------------------------------------------
results <- read_rds("Results/Sensitivity_analysis_results_analytical_period.rds") %>%
  filter(direction == 1) %>%
...
fa <- ggplot(filter(results, path %in% c("ssb -> age1", 
                                         "age1 -> age2", 
                                         "age2 -> rec"))) +
  geom_linerange(aes(x = start_year, ymin = -Inf, ymax = Inf, color = significance),
    linewidth = 0.75, show.legend = FALSE
  ) +
  scale_color_manual(values = c(
    "significant" = alpha("white", 0.25),
    "not significant" = alpha(mypal[6], 0.25)
  )) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 1981, linetype = "dashed") +
  geom_ribbon(aes(x = start_year, ymin = Estimate - 2 * Std_Error, ymax = Estimate + 2 * Std_Error),
    alpha = 0.125
  ) +
  geom_line(aes(x = start_year, y = Estimate)) +
  facet_wrap(~path, ncol = 3, labeller = labeller(path = supp.labs)) +
  scale_x_continuous("Start year", limits = c(1961, 2001)) +
...
fb <- ggplot(filter(results, path %in% c("sst -> ssb", 
  ...
  geom_line(aes(x = start_year, y = Estimate), color = mypal[2]) +
  facet_wrap(~path, ncol = 3, labeller = labeller(path = supp.labs)) +
  scale_x_continuous("Start year", limits = c(1961, 2001)) +
  scale_y_continuous("Coefficient", breaks = c(-1, 0, 1)) +
...
fc <- ggplot(filter(results, path %in% c("mackerel_ssb -> age1", 
  ...
  geom_line(aes(x = start_year, y = Estimate), color = mypal[3]) +
  facet_wrap(~path, ncol = 3, labeller = labeller(path = supp.labs)) +
  scale_x_continuous("Start year", limits = c(1961, 2001)) +
  scale_y_continuous("Coefficient", breaks = c(-1, 0, 1)) +
  # scale_color_manual(values = MetBrewer::met.brewer("OKeeffe2")) +
...
fd <- ggplot(filter(results, path %in% c("fm -> ssb"))) +
  ...
  geom_line(aes(x = start_year, y = Estimate), color = mypal[1]) +
  facet_wrap(~path, ncol = 3, labeller = labeller(path = supp.labs)) +
  scale_x_continuous("Start year", limits = c(1961, 2001)) +
  scale_y_continuous("Coefficient", breaks = c(-1, 0, 1)) +
  # scale_color_manual(values = MetBrewer::met.brewer("OKeeffe2")) +
...
ggsave("Figures/sensitivity_analytical_period.pdf", device = cairo_pdf, width = 6, height = 9)
